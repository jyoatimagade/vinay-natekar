            ...
            //<https: //github.com/PHPMailer/PHPMailer/issues/2298>), and
                ...
                */
                ...
                for ($i = 0; $i < $length; ++$i) { $c=$string[$i]; //All other characters have a special meaning in at
                    ...
                    $readable=file_exists($path); //If not a UNC path (expected to start with \\), check read
                    permission, see #2069 if (strpos($path, '\\\\' ) !==0) { $readable=$readable && is_readable($path);
                    } return $readable; } /** * Send mail using the PHP mail() function. * * @see
                        ...
                        */
                        ...
                        !preg_match('/^(?P<lang>[a-z]{2})(?P<script>
                            ...
                            class SMTP {
                                ...
                                function connect($host, $port = null, $timeout = 30, $options = []) {
                                ...
                                function getSMTPConnection($host, $port = null, $timeout = 30, $options = []) {
                                    static $streamok;
                                ...
                                function authenticate(
                                    ...
                                    if (array_key_exists('EHLO', $this - > server_caps)) {
                                        ...
                                        $this - > edebug('Auth method requested: '.($authtype ? : 'UNSPECIFIED'), self::
                                            DEBUG_LOWLEVEL);
                                        $this - > edebug(
                                            'Auth methods available on the server: '.implode(',', $this - >
                                                server_caps['AUTH']),
                                            self::DEBUG_LOWLEVEL
                                        );
                                        ...
                                        if (null !== $authtype && !in_array($authtype, $this - > server_caps['AUTH'],
                                                true)) {
                                            $this - > edebug('Requested auth method not available: '.$authtype, self::
                                                DEBUG_LOWLEVEL);
                                            $authtype = null;
                                        }
                                ...
                                function sendHello($hello, $host) {
                                    $noerror = $this - > sendCommand($hello, $hello.
                                        ' '.$host, 250);
                                ...
                                function client_send($data, $command = '') {
                                ...
                                function get_lines() {
                                    ...
                                    while (is_resource($this - > smtp_conn) && !feof($this - > smtp_conn)) {
                                        ...
                                        if (!$n) {
                                            $this - > edebug(
                                                'SMTP -> get_lines(): select timed-out in ('.$this - > Timelimit.
                                                ' sec)',
                                                self::DEBUG_LOWLEVEL
                                            );
                                            break;
                                        ...
                                        $this - > edebug('SMTP INBOUND: "'.trim($str).
                                            '"', self::DEBUG_LOWLEVEL);
                                        $data. = $str;
                                ...
                                function recordLastTransactionID() {
                                    ...
                                    if (empty($reply)) {
                                        ...
                                        foreach($this - >
                                            smtp_transaction_id_patterns as $smtp_transaction_id_pattern) {
                                            $matches = [];
